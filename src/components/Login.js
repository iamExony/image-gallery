...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-cover bg-center bg-no-repeat" style={{ backgroundImage: 'url("/background.jfif")' }}>
     <div className="max-w-lg mx-auto  my-4 px-16 py-12 space-y-8 backdrop-blur-md bg-white/30 rounded-xl drop-shadow-4xl">
        <div>
          <h2 className="mt-6 text-center text-2xl md:text-3xl font-extrabold text-black">
            {login ? (
              <h2 className="text-black">Welcome to Gallery</h2>
            ) : (
              <>
                <h2 className="text-black">Welcome to Gallery</h2>
                <span className="text-sm text-black">Get Awesome Photo Gallery</span>
              </>
            )}
          </h2>
        </div>
        <form className="mt-8 space-y-6" onSubmit={(e) => handleSubmit(e, login ? 'signin' : 'signup')}>
          <div className="rounded-md shadow-sm -space-y-px">
            <div>
              ...
              <input
                ...
                className="appearance-none relative block w-full px-3 py-2 border rounded-md placeholder-gray-400 focus:outline-none focus:ring focus:border-blue-500 focus:z-10 sm:text-sm"
          ...
          <div className="flex items-center justify-center text-black text-lg font-semibold">
            <p className={login ? 'hidden' : ''}>
              Already a Member?{' '}
              <span onClick={() => setLogin(true)} role="button" className="hover:text-red-900 cursor-pointer">
                Sign In
              </span>
            </p>
            <p className={login ? '' : 'hidden'}>
              New User?{' '}
              <span onClick={() => setLogin(false)} role="button" className="hover:text-red-900 cursor-pointer">
                Sign Up
              </span>
            </p>
          </div>
